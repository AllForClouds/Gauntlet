...
for i in range(0, commentNum) :
    ...
    if commentLink > 0 :
        count=count+1
        #print(i,commentLink)
        commentTab = comments[i].find_element_by_class_name("lite-iconf-comments")
        driver.execute_script("arguments[0].scrollIntoView();", commentTab)
        driver.execute_script('window.scrollBy(0,-100)')
        time.sleep(0.5)
        commentTab.click()
        time.sleep(0.5)
        driver.find_element_by_xpath('//*[@id="app"]/div[1]/div/main/div[1]/div/span/textarea[1]').clear();
        driver.find_element_by_xpath('//*[@id="app"]/div[1]/div/main/div[1]/div/span/textarea[1]').send_keys("不退让");
        time.sleep(2)
